...
def save_attendance(player_name, new_status, target_date=None):
    ...
    costs = {"In": 1, "Double Session": 2, "Out": 0}
    new_cost = costs.get(new_status, 0)
    ...
    try:
        ...
        if existing.data:
            ...
            old_cost = costs.get(old_status, 0)
            adjustment = old_cost - new_cost
            
            # force overwrite the existing row
            conn.table("player_credits").update({"remaining_sessions": current_sessions + adjustment}).eq("player", player_name).execute()
            conn.table("attendance").update({"status": new_status}).eq("player", player_name).eq("date", target_date).execute()
        else:
            # New entry: subtract the cost
            conn.table("player_credits").update({"remaining_sessions": current_sessions - new_cost}).eq("player", player_name).execute()
